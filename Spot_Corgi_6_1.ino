...
int walkRate = 15;                      // Determine pace of servo moves during Walk
...
int liftSize = 10;                          // Lower leg raise distance
int stepSize = 28;                          // Upper leg move distance. Array fill crashes if > 48. Don't know why.
...
String moves[5] = {"Start", "Crouch", "Sit", "Stand", "Shake"};
//                    0  1  2  3  4  5  6  7
...
int moveTable[6][8]  = {                                  //Servo settings for preset positions
  ...
  {100,  80, 100,  80,  0, 180, 180,  0},   //Stand    Right Arrow
  { 35, 155,  35, 155, 160, 130, 170, 0},   //Shake    Display
  {100,  80, 100,  80,  0, 180, 180,  0}    //Stand values backup for reset
};
...
void setup()
{
  ...
  Serial.println("SpotCorgi 6.1 test!");
  ...
  // doMove(1);                         // Move to Crouch position
  delay(1000);
...
int fillwalkTable()                                    // Writes array of positions all servos to Walk
{
  ...
  for (int m = 4; m < moveCount; m++) {               // Rebuild array to allow for delayed start of each leg
    // legArc- Circular function to adjust low leg angle keep body height constant as upper leg moves back
    legArc[m] = sqrt(pow(((moveCount - 4) / 2), 2) - pow((((moveCount - 4) / 2) - m + 4), 2));
    Serial.print("moveNum = ");
    Serial.print(moveNum);
    Serial.print("  legArc =");
    Serial.println(legArc[m]);
  }

  for (int m = 0; m < moveCount; m++) {               // Duplicate array to allow for delayed start of each leg
...
int tweakVariables(int serv, int fb) {       // Adjust walk variables using remote
  ...
  for (int i = 0; i < 2; i++) {              // Set servos
    ...
    moveTable[3][i + serv] = moveTable[3][i + serv] + legAdjust[i + serv];
  }
  //  fillwalkTable();
...
int displayResults(int move) {
  ...
  for (s = 0; s < numServos; s++) {
    ...
    Serial.print(s);
    Serial.print(" srv_pos = ");
    Serial.println(srv_pos[s]);
  }
  Serial.println(" ");
}
//************************************************************************
int allDone()                 // Check if all servos have reached their finish position
{
  int c = 0;
